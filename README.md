...
- **`getMyLimits()` nunca lanza una excepción** aunque el RPC falle (por ejemplo, si la migración 0005 aún no se ha ejecutado en tu proyecto de Supabase) — degrada a los límites de "Registrado" en vez de tirar toda la página con el error boundary genérico, y lo marca con `isFallback: true`. Cuando esto pasa, el dashboard y `/dashboard/settings` muestran un aviso visible en rojo ("No pudimos cargar tus límites reales…") en vez de fallar en silencio, además de registrarlo en los logs del servidor (busca "get_my_limits RPC falló").
